...
For those unaware, #link("https://github.com/voxell-tech/motiongfx")[MotionGfx] is a backend-agnostic motion graphics creation framework. It uses a command-based workflow to generate animations (similar to Manim and Motion Canvas). We're working hard on an editor and LLM integration.

= The old guard: After Effects

For many years the industry has been stuck with #link("https://www.adobe.com/my_en/products/aftereffects.html")[After Effects]. It's amazing, even effective when it comes to motion graphics.
However, throughout my usage of the software, I've run into 3 recurring bottlenecks:
...
Both, however, did not address the keyframe problem that #link("https://github.com/motion-canvas/motion-canvas")[Motion Canvas] and #link("https://www.manim.community")[Manim] resolve beautifully.
...
#{
  let diagram(c) = {
    ...
    html.frame(box(
      width: track-x0 + track-w + 2em,
      height: tracks.len() * row-h,
    )[
...
#{
  let diagram(c) = {
    ...
    html.frame(box(width: steps.len() * cell, height: box-h + 2.4em)[
      ...
      #place(
        top + left,
        dx: playhead-i * cell + step-w / 2 - 1.5em,
        dy: -1.5em,
      )[
        #text(size: 0.8em, fill: c.text, weight: "bold")[playhead]
      ]
...
This is how you create animations in Manim, Motion Canvas, or #link("https://github.com/voxell-tech/motiongfx")[MotionGfx] under the hood! Animations are created as a sequence of actions / commands and layered on top of one another.
...
Hera generates animations entirely using AI, with some room to make limited changes, like the color of an object. Changing much more requires regenerating the entire animation.
To me, it's just not an editor. You need to have full control over your timeline structure to be able to make custom modifications. No serious animators would want a prompt-only editor. In my eyes, it's set to fail. Not that it won't make money, but that it can't scale.
...
I'll admit, creating a motion editor while trying to compete with this industry-leading software is extremely challenging. But that's not a reason for us not to try! After all, _*we're born to create, and create we will!*_
